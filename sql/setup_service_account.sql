...
-- Note: Hybrid tables require PRIMARY KEY constraints and do not support CLUSTER BY clauses
-- Hybrid tables use PRIMARY KEY constraints for performance optimization
-- They provide row-level locking and enforce constraints for transactional workloads
...
BEGIN
    INSERT INTO API_AUDIT_LOG (
        REQUEST_ID, ENDPOINT_ID, API_KEY_ID, REQUEST_METHOD, 
...
$$
BEGIN
    MERGE INTO API_USAGE_LOG AS target
    USING (SELECT API_KEY_ID, ENDPOINT_ID, CURRENT_TIMESTAMP() AS LAST_USED) AS source
    ON target.API_KEY_ID = source.API_KEY_ID AND target.ENDPOINT_ID = source.ENDPOINT_ID
    WHEN MATCHED THEN
        UPDATE SET 
