...
-- Check clustering information for API_USAGE_LOG
SELECT SYSTEM$CLUSTERING_INFORMATION('API_USAGE_LOG');
...
WHERE TABLE_SCHEMA = 'PUBLIC'
  AND TABLE_NAME IN ('API_AUDIT_LOG', 'API_USAGE_LOG')
ORDER BY BYTES DESC;
...
-- Check clustering depth for API_USAGE_LOG
SELECT 
    'API_USAGE_LOG' AS TABLE_NAME,
    CLUSTERING_KEY,
    TOTAL_CLUSTER_COUNT,
    AVERAGE_OVERLAPS,
    AVERAGE_DEPTH,
    CASE 
        ...
        ELSE 'Poor - Consider reclustering'
    END AS CLUSTERING_QUALITY
FROM TABLE(SYSTEM$CLUSTERING_INFORMATION('API_USAGE_LOG'));
...
FROM (
    ...
    FROM TABLE(SYSTEM$CLUSTERING_INFORMATION('API_AUDIT_LOG'))
    UNION ALL
    SELECT 'API_USAGE_LOG' AS TABLE_NAME, AVERAGE_DEPTH
    FROM TABLE(SYSTEM$CLUSTERING_INFORMATION('API_USAGE_LOG'))
);
...
WHERE USER_NAME = 'API_PROXY_SERVICE_MANAGER'
...
BEGIN
    ...
    -- Check API_USAGE_LOG clustering
    ...
    -- Recluster API_USAGE_LOG if needed
    IF token_depth > 4.0 THEN
        ALTER TABLE API_USAGE_LOG RECLUSTER;
        result := result || 'API_USAGE_LOG reclustered. ';
    END IF;
